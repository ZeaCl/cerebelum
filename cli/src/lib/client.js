/**
 * Cerebelum CLI client — config & auth management.
 *
 * Reads the shared ZEA config at ~/.config/zea/config.json
 * (same file used by zea-cli and zea-thalamus).
 *
 * Token priority: ZEA_PAT → THALAMUS_PAT → ZEA_TOKEN → config.token
 * API URL priority: CEREBELUM_API_URL → CEREBELUM_URL → config.cerebelumUrl → default
 */
...
/**
 * Get a configured client for Cerebelum API calls.
 *
...
export async function getClient(allowUnauthenticated = false) {
  ...
  if (!token && !allowUnauthenticated) {
    throw new Error('Not authenticated. Please run "zea login" or set ZEA_PAT.');
  }
