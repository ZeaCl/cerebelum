/**
 * cerebelum config — Manage Cerebelum configuration.
 *
 * Reads/writes the shared ZEA config file (~/.config/zea/config.json).
 * Manages cerebelumUrl and other cerebelum-specific settings.
 */
...
export function register(program) {
  const configCmd = program.command('config').description('Manage Cerebelum configuration');
    ...
    .action(async (envName) => {
      try {
        ...
        if (envName === 'local') {
          ...
          console.log(chalk.green('✅ Cerebelum environment set to LOCAL'));
        } else if (envName === 'prod') {
          config.cerebelumUrl = 'https://cerebelum.zea.cl';
          config.apiUrl = 'https://auth.zea.cl';
          console.log(chalk.green('✅ Cerebelum environment set to PROD'));
        } else {
