defmodule Cerebelum.CapitalCallWorkflow do
  ...
  defp api(method, path, body, ctx) do
    ...
    req =
      if method in [:get, :head],
        ...
        else: {String.to_charlist(url), headers, ~c"application/json", payload}
  ...
  def pay_items(ctx, {:ok, _sc}, {:ok, cc}) do
    ...
    if a == "pay" do
      ...
      api(
        :post,
        "/capital-calls/#{cc.capital_call_id}/pay",
        %{
          item_id: item_id,
          amount: d["amount"] || "0",
      ...
      # TODO: N+1 — GET all items after every pay. Optimize with counter or response field.
  ...
  # TODO: placeholder — implement actual notification (email, webhook)
  ...
  defp wfa(step, cc, status, actions) do
    {:wait_for_approval, [type: :manual],
     %{
       step: step,
       capital_call_id: cc.capital_call_id,
       status: status,
